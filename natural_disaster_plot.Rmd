...
hc <- nd %>% 
  ...
  )%>%   hc_legend(title=list(text='<span style="font-size: 9px; color: #666; font-weight: normal">(Click to hide; ctrl-click/command-click to filter/show all;)</span>')) %>% 
  hc_plotOptions(
  series = list( 
    events = list(
  ...
  htmlwidgets::saveWidget('naturaldisaster.html',selfcontained = FALSE)
hc 
```
...
```{r}
p<-ggplot(actions) +
  geom_segment( aes(x=Actions, xend=Actions, y=Lowest.range.of.co2.reduction, yend=Highest.range.of.co2.reduction), color="grey") +
  geom_point( aes(x=Actions, y=Lowest.range.of.co2.reduction,color=I("#F78C6B")), size=3 ) +
  geom_point( aes(x=Actions, y=Highest.range.of.co2.reduction,color=("#073B4C")), size=3 ) +
  coord_flip()+
  scale_y_sqrt()+
  xlab("") +
  ylab("Approximate CO2 reduced per year (kg)")  +
  labs(title="Recommended individual actions to reduce greenhouse gas emissions") +
  theme_minimal() +
  theme(
    legend.position = "left",
    plot.title = element_text(size= 14, hjust=0.01, margin = margin(b = -0.1, t = 0.4, l = 2, unit = "cm"))
  )
ggplotly(p, width=1050)
...
fig <- plot_ly(actions, color = I("Gray8"),width=1000,height=500)
...
fig <- fig %>% layout(
  autosize = F,height=470,
   margin=list(t=70),
   paper_bgcolor= "rgba(0,0,0,0)",
  plot_bgcolor="rgba(0,0,0,0)",
    title = list(text="Recommended individual actions to reduce greenhouse gas emissions",x=0.5,font=list(size=22)),
  legend = list(orientation = 'h', x=0.1, y=-0.6),
    ...
    xaxis = list(title = "Approximate CO2 reduced per year (kg)",
                 range=c(-1000,120000),
                 rangeslider=list(range=c(-5, 120000),thickness=0.2, borderwidth=1)
                 ),
